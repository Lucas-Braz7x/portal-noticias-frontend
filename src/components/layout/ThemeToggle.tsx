...
export function ThemeToggle() {
  ...
  const theme = themeOverride ?? (mounted ? getPreferredTheme() : 'light');
  ...
  const toggleTheme = () => {
    ...
    const currentTheme = themeOverride ?? getPreferredTheme();
  ...
  return (
    <button
      type="button"
      className={mounted ? `${styles.toggle} ${styles.ready}` : styles.toggle}
      onClick={toggleTheme}
      aria-label={label}
      title={label}
    >
