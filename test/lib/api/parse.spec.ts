...
describe('parseApiResponse', () => {
  ...
  it('throws ApiValidationError when schema does not match', () => {
    ...
    expect(() => parseApiResponse(articleSummarySchema, invalidData, 'listArticles')).toThrow(
      ApiValidationError,
    );
    ...
    try {
      ...
      expect((error as ApiValidationError).message).toContain('Invalid API response');
    }
  });
  ...
  it('includes field paths in error message', () => {
    ...
    expect(() => parseApiResponse(schema, { name: 123 }, 'test')).toThrow(/name/);
  });
});
