...
describe('Pagination', () => {
  it('renders page size selector even when totalPages is 1', () => {
    render(<Pagination meta={{ ...meta, page: 1, totalPages: 1 }} listParams={listParams} />);

    expect(screen.getByLabelText('Itens por página')).toHaveValue('20');
    expect(screen.queryByRole('link', { name: 'Ir para página 2' })).not.toBeInTheDocument();
  });
  ...
  it('marks the current page with aria-current', () => {
    ...
    expect(screen.getByLabelText('Página 2, página atual')).toHaveAttribute('aria-current', 'page');
  });

  it('builds links preserving active filters and limit', () => {
    render(<Pagination meta={meta} listParams={listParams} />);

    expect(screen.getByRole('link', { name: 'Ir para página 1' })).toHaveAttribute(
      'href',
      '/?q=ia&category=tecnologia&limit=20',
    );
    expect(screen.getByRole('link', { name: 'Ir para página 3' })).toHaveAttribute(
      'href',
  ...
  it('navigates without browser scroll', () => {
    ...
    expect(push).toHaveBeenCalledWith('/?q=ia&category=tecnologia&page=3&limit=20', {
      scroll: false,
    });
  });
  ...
  it('resets to page 1 when page size changes', () => {
    ...
    fireEvent.change(screen.getByLabelText('Itens por página'), { target: { value: '30' } });

    expect(push).toHaveBeenCalledWith('/?q=ia&category=tecnologia&limit=30', { scroll: false });
  });
